...
serve(async (req) => {
  ...
  try {
    ...
    const TRANSCRIPT_SUMMARY_PROMPT = `다음은 테니스 레슨 녹음 전체 전사입니다.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ 절대 원칙 — 위반 시 출력 전체 무효
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 코치가 전사에서 실제로 사용한 단어·표현만 그대로 옮길 것
■ 재해석·요약·번역·일반화 금지. "헤드 앞으로"를 "컨트롤 연습"으로 바꾸는 것처럼 코치의 말을 다른 표현으로 대체하지 말 것
■ 전사에 없는 기술명·동작·개념을 추가하거나 추론하지 말 것
■ lesson_flow: 코치가 한 말을 순서대로 직접 기록. 요약이나 재표현 금지.

지침:
1. lesson_flow에는 코치의 지도 내용만 포함 (사적 대화, 날씨, 잡담 제외)
2. key_techniques와 coach_instructions는 전사에 실제 나온 단어만 사용
3. lesson_content_ratio: 전체 녹음 중 레슨 관련 대화 비율(0~1)

출력 형식 (JSON):
{
  "key_techniques": ["전사에 나온 기술어 그대로"],
  "main_issues": ["전사에서 코치가 지적한 문제점 그대로"],
  "lesson_flow": "코치가 한 말 순서대로 직접 기록 (재표현 금지)",
  "coach_instructions": ["코치 지시사항 원문 그대로"],
...
- summary: 코치가 "오늘 레슨 요약"에서 사용한 단어와 표현만 사용할 것. 재해석·일반화 금지. 코치가 "헤드 앞으로"라고 말했으면 그대로 쓸 것 — "컨트롤 향상"처럼 다른 말로 바꾸지 말 것.
- session_goals, improvement_points, next_goals: "오늘 레슨 요약"에 코치가 직접 언급한 내용만 작성. 추론하거나 일반화하지 말 것.
- drill_suggestions (예외): 오늘 레슨 요약의 기술·문제점을 기반으로 "관련 교육 자료"에서 적합한 드릴을 선택. 단, 레슨에서 다루지 않은 완전히 새로운 기술(예: 레슨에서 풋워크 언급이 없는데 풋워크 드릴 추천)은 금지.
- 레슨 요약에 없는 내용은 분석에 절대 반영하지 마세요.
